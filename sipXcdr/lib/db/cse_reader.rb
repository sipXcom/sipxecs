...
class CseReader < Dao
  ...
  def run(cse_queue, start_id, start_time, stop_time = nil)
    @log.debug("cse_reader.rb:: Connecting CSE database. #{@database_url}") if @log
    connect do | dbh |
      if stop_time
        check_purge(dbh)
        ...
        loop do
          ...
          if ((first_id.to_i + MAX_CSES) > @last_read_id.to_i)
             @log.debug("cse_reader.rb:: Going to sleep. Connection #{@database_url.host}:#{@database_url.port}") if @log
             break if @stop.wait
             @log.debug("cse_reader.rb:: Waking up. Connection #{@database_url.host}:#{@database_url.port}") if @log
          end
        end
      end
    end
  rescue DBI::DatabaseError, DBI::OperationalError, DBI::ProgrammingError => excp
    @log.error("cse_reader.rb:: Loss of connection to database #{@database_url} - retrying to connect after sleep") if @log
    if !@stop.wait
       dbh.disconnect
       retry
    end
  rescue
    @log.error("cse_reader.rb:: Exception in reader thread: <#{$!}> - Database = #{@database_url}") if @log
    # save current exception so that we can re-raise it
    Thread.current[:exception] = $!
    raise
  ensure
    @log.debug("cse_reader.rb:: Stopping CSE reader - #{@database_url.host}:#{@database_url.port}.") if @log
  end

  def stop
    @log.debug("cse_reader.rb:: CSE Reader - #{@database_url.host}:#{@database_url.port} - calling stop") if @log
    @stop.stop
  end
