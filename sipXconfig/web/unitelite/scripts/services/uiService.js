  ...
  uw.service('uiService', [
    ...
    function ($rootScope, $interval, $sce, sharedFactory, restService, _, CONFIG) {
      ...
      var secondary = {
        ...
        callhistory: {
          startTime: '2010-11-01T10:00',
          endTime: '2016-11-01T12:00',
          calls:[],
          number:null,
          availableOptions: [
            {id: '1', name: '- all -'},
          ...
          apply: function () {
            ...
            restService.getCallHistory().then(function (data) {
              //filter by date
              var endDate = new Date(secondary.callhistory.endTime.split('T').join(' '));
              var startDate = new Date(secondary.callhistory.startTime.split('T').join(' '));
        ...
        settings: {
          ...
          speed: {
            ...
            initDials: true,
            getPattern: function() {
              if (secondary.settings.speed.initDials == true)
                return /.*/;
              else
                return /(^\d+|(^(\w+)@([a-zA-Z0-9_.]+)))$/;
            },

            init: function () {
              secondary.settings.speed.initDials = true;
              secondary.settings.errors.notAvailable = null;
            ...
            addEntry: function () {
              ...
              secondary.settings.speed.initDials = false;
              return;
            },
