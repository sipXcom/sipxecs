...
#include "sqa/BlockingTcpClient.h"
...
const std::string& BlockingTcpClient::className()
{
...
const std::string & BlockingTcpClient::getServicePort() const
{
  return _servicePort;
}

const std::string & BlockingTcpClient::getServiceAddress() const
{
  return _serviceAddress;
}

BlockingTcpClient::BlockingTcpClient(
...
void BlockingTcpClient::onReadTimeout(const boost::system::error_code& e)
{
  if (e)
  {
    return;
...
void BlockingTcpClient::onWriteTimeout(const boost::system::error_code& e)
{
  if (e)
  {
    return;
...
void BlockingTcpClient::onConnectTimeout(const boost::system::error_code& e)
{
  if (e)
  {
    return;
...
bool BlockingTcpClient::timedWaitUntilDataAvailable(boost::function<void(const boost::system::error_code&)> onTimeoutCb,
                                  int timeoutMs,
...
bool BlockingTcpClient::timedWaitUntilReadDataAvailable()
{
...
bool BlockingTcpClient::timedWaitUntilWriteDataAvailable()
{
...
bool BlockingTcpClient::connect(const std::string& serviceAddress, const std::string& servicePort)
{
...
bool BlockingTcpClient::send(const StateQueueMessage& request)
{
  assert(_pSocket);
...
bool BlockingTcpClient::receive(StateQueueMessage& response)
{
  assert(_pSocket);
...
bool BlockingTcpClient::sendAndReceive(const StateQueueMessage& request, StateQueueMessage& response)
{
  if (send(request))
...
unsigned long BlockingTcpClient::getNextReadSize()
{
  short version = 1;
...
bool BlockingTcpClient::isConnected() const
{
  return _isConnected;
}

std::string BlockingTcpClient::getLocalAddress()
{
  try
  {
    if (!_pSocket)
